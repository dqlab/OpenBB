...
def vector_values(value: Any, expected: int, operation: str) -> list[float]:
    ...
    if isinstance(data, Sequence) and not isinstance(data, (str, bytes)):
        ...
        try:
            ...
            raise DQLibExecutionError(f"dqlib returned an invalid vector for {operation}.") from exc
...
def pricing_values(response: Any, operation: str) -> tuple[float, float | None, str]:
    ...
    if results is None or not hasattr(results, "present_value"):
        raise DQLibExecutionError(f"dqlib returned invalid pricing results for {operation}.")
    listed_fields = {descriptor.name for descriptor, _value in results.ListFields()}
    cash_value = float(results.cash_value) if "cash_value" in listed_fields else None
...
def build_option_settings(
    ...
    if domain not in risk_functions:
        raise DQLibExecutionError(f"Typed option settings are unavailable for dqlib.{domain}.")
